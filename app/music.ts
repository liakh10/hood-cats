// Procedural loop — a tense heist pulse: syncopated minor bass, a clipped
// stab, and a tight hi-hat tick, built live with WebAudio (no mp3). Started
...
export class MusicEngine {
  ...
  private readonly bass = [110, 110, 130.81, 110, 98, 98, 110, 87.31]; // A minor-ish walk
  private readonly stab = [440, 523.25, 622.25, 523.25];
  ...
  private tickFn = () => {
    ...
    this.note(this.bass[this.step % this.bass.length], t, "sawtooth", 0.07, 0.16, 500);
    if (this.step % 4 === 2) this.note(this.stab[Math.floor(this.step / 4) % this.stab.length], t, "square", 0.03, 0.1, 1800);
